...
def get_memory_usage_mb():
    process = psutil.Process(os.getpid())   # Get the current process using its PID (process ID)
    return process.memory_info().rss        # Return the memory used by the process in bytes
...
def main():
    ...
    for _ in range(num_runs):
        for folder_name in input_folders:
            ...
            memory_usage_bytes = end_mem - start_mem            # Calculate the memory usage in bytes
            ...
            # -------- PRINT METRICS --------
            print(f"python,{folder_name},{NUM_FILE}doc,{execution_time_ms:.4f},{memory_usage_bytes},{ts_str}")

            # Append the metrics to a result file
            with open(f"resultsPython.csv", 'a', encoding='utf-8') as f:
                f.write(
                    f"python,{folder_name},{NUM_FILE}doc,{execution_time_ms:.4f},{memory_usage_bytes},{ts_str}\n"
                )
