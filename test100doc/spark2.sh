...
for (( RUN_ID=1; RUN_ID<=TIMES; RUN_ID++ )); do
    ...
    OUTPUT_DIR="outputSpark2_100doc/${OUTPUT_BASE_INDEX}"

    spark-submit --master yarn --conf spark.executor.processTreeMetrics.enabled=true ../spark/sparkInvertedIndex2.py "$INPUT_DIR" "$OUTPUT_DIR" -1 > spark_log.txt 2>&1
    ...
    echo "$TS,spark,$REAL_PARTITION,$DRIVER_MAX_MEMORY,$EXEC1_DATA,$EXEC2_DATA,$ALLOC,$DIFF_MS" >> spark2.csv
