...
SYSTEM_COLORS = {
    ...
    'Spark wholeTextFiles':         'tab:green',
    'Spark read':         'tab:red',
}
...
def preprocess_all_data():
    dfs = [
        load_filtered_csv('hadoop2.csv', 
                          'Hadoop', reducer=5),
        load_filtered_csv('hadoopInMapperCombiner2.csv', 
                          'Hadoop-InMapper', reducer=5),
        load_filtered_csv('spark2.csv', 
                          'Spark read', input_partitions=-1),
        load_filtered_csv('spark.csv', 
                          'Spark wholeTextFiles', input_partitions=-1),
    ...
    agg = df.groupby(['system', 'SIZE_BYTES'], as_index=False).agg({
        'TIME_MS': 'mean'
    })

    agg.sort_values(['SIZE_BYTES', 'system'], inplace=True)
    return agg
...
def plot_for_doc(agg, doc_label):
    ...
    labels = sizes #[BYTES_TO_LABEL(sz) for sz in sizes]
    systems = ['Hadoop', 'Hadoop-InMapper', 'Spark wholeTextFiles', 'Spark read']
    ...
    for i, system in enumerate(systems):
        ...
        for sz in sizes:
            ...
            times.append(row['TIME_MS'].values[0] if not row.empty else 0)
        ax.bar(
    ...
    ax.set_title(f'Execution Time Comparison — with 100 files as input (2GB)')
    ax.legend()
    ax.grid(axis='y', linestyle='--', alpha=0.5)
    plt.tight_layout()

    os.makedirs(OUTPUT_DIR, exist_ok=True)
    plt.savefig(os.path.join(OUTPUT_DIR, f'comparison_{doc_label}.png'))
    plt.close()
