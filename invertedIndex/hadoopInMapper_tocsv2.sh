...
DOCS=(20doc)   #(10doc 20doc)
SIZES=(512KB 1MB 512MB 1GB 2GB)
NUMBER_REDUCERS=(5 15)    #(1 5 15)
OUTPUT_BASE_INDEX=152
    ...
    for REDUCERS in "${NUMBER_REDUCERS[@]}"; do
        ...
        for (( RUN_ID=1; RUN_ID<=TIMES; RUN_ID++ )); do
            ...
            for SIZE in "${SIZES[@]}"; do
                ...
                OUTPUT_DIR="outputInMapper_${DOC}/${OUTPUT_BASE_INDEX}_${SIZE}"
                ...
                echo "$TS,hadoop,$SIZE,$DOC,$SPLIT,$REDUCERS,$PHYSICAL_MEMORY,$PEAK_MAP_PHYSICAL_MEMORY,$PEAK_REDUCE_PHYSICAL_MEMORY,$ALLOC,$DIFF_MS" >> hadoopInMapperCombiner2.csv

                echo "Results for $INPUT_DIR saved in hadoopInMapperCombiner2.csv"
                ((OUTPUT_BASE_INDEX++))
            done
        done
    done
