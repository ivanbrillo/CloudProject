...
# === Constants ===
OUTPUT_DIR = './'
FILENAME = '../../invertedIndex/hadoopInMapperCombiner.csv'
SIZE_MAPPING = {
    '512KB': 512 * 1024,
    '1MB': 1 * 1024 * 1024,
    '512MB': 512 * 1024 * 1024,
...
def plot_line_charts(agg, metric, ylabel, file_prefix):
    for dataset in agg['doc'].unique():
        ...
        title = f"HADOOP IN-MAPPER COMBINER: {'Execution Time' if metric == 'time' else 'Memory Usage'} (with {dataset} as input)"
        plt.title(title)
...
def plot_comparison_bars(agg, metric, ylabel, title, fname):
    ...
    handles = []
    for r in reducers:
        handles.append(Patch(facecolor=BASE_COLORS[r], 
        edgecolor=BASE_COLORS[r], 
        label=f'{r} Reducers'))
    for ds in docs:
...
def main():
    ...
    plot_comparison_bars(
        ...
        title='HADOOP IN-MAPPER COMBINER: Execution Time Comparison',
    ...
    plot_comparison_bars(
        ...
        title='HADOOP IN-MAPPER COMBINER: Memory Usage Comparison',
