...
def preprocess_all_data():
    dfs = [
        load_filtered_csv('../../invertedIndex/hadoop.csv', 
                          'Hadoop', reducer=5),
        load_filtered_csv('../../invertedIndex/hadoopInMapperCombiner.csv', 
                          'Hadoop-InMapper', reducer=5),
        load_filtered_csv('../../spark/spark.csv', 
                          'Spark', input_partitions=-1),
        load_filtered_csv('../../python/resultsPython.csv', 
                          'Python')
    ]
    df = pd.concat(dfs, ignore_index=True)
    df['SIZE_BYTES'] = df['size'].map(SIZE_MAPPING)
    df['TIME_MS'] = df['time']  # assuming già in ms
    # Media per system, doc e dimensione
    agg = df.groupby(['system', 'doc', 'SIZE_BYTES'], as_index=False).agg({
        'TIME_MS': 'mean'
    })
    agg.sort_values(['doc', 'SIZE_BYTES', 'system'], inplace=True)
    return agg


def plot_for_doc(agg, doc_label):
    subset = agg[agg['doc'] == doc_label]
    sizes = sorted(subset['SIZE_BYTES'].unique())
    labels = [BYTES_TO_LABEL(sz) for sz in sizes]
    systems = ['Hadoop', 'Hadoop-InMapper', 'Spark', 'Python']
    ...
    ax.set_title(f'Execution Time Comparison — with {doc_label} as input')
    ax.legend()
